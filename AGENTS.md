...
## Migrations workflow
Numbered SQL files in `supabase/migrations/` (currently through `00039`). **Drew runs them manually in the Supabase SQL editor** — always tell him which numbers to run. Settings pages render a "One-time setup needed" card if their table is missing. Verify applied via REST probes (`?select=col&limit=1` errors on missing). As of handoff: 00035 (document view tracking + visible_to_client) and 00039 (branded_shell column) may still be PENDING — verify before relying on them.
...
1. **Phase 3 after-sign automation**: document signed → booking-helper-style actions (auto status → Booked, confirmation email, retainer request). Hook point: `signDocument()` in `src/app/sign/[token]/actions.ts`.
2. **SMS via Twilio** (completes the helper flow: email + SMS + status change). Needs Drew's Twilio account/creds.
3. **PayPal API** payments (client pays from the sign/thank-you flow; after-payment internal actions).
4. Client portal (documents `visible_to_client` flag already exists), manual compose/send-one-off email UI, employee portal features (check-in, timesheets, confirm/decline — staff_settings already stores all the config), inbox + nav counters, Custom Fields settings page (last placeholder).
5. Verify migrations 00035/00039 applied.
