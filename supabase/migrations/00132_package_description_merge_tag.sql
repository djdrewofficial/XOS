-- Add the <package_description> merge tag — the selected package's description
-- (the "what's included" list). Stored as plain text on packages.description;
-- we convert line breaks to <br> so multi-line lists render in emails/contracts.
...
select 'package_description',
       'Package Description',
       'Event',
       'The selected package''s description (the "what''s included" list).',
       true, 'builtin', true, 0
where not exists (select 1 from merge_tags where tag_key = 'package_description');
