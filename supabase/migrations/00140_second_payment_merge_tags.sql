-- Add <second_payment_amount> and <second_payment_date> — the event's SECOND
-- scheduled payment (by seq). The first scheduled payment is already exposed via
-- <retainer_amount>/<retainer_due_date>. Both render blank when there is no
-- second installment.
...
begin
  ...
  if position('<discount>' in out_text) > 0 then
    select string_agg(
             'Discount'
             || case when coalesce(nullif(d.lbl, ''), '') <> '' then ' (' || d.lbl || ')' else '' end
...
select 'second_payment_amount',
       'Second Payment Amount',
       'Event',
       'The amount of the event''s second scheduled payment (blank if none).',
       true, 'builtin', true, 0
where not exists (select 1 from merge_tags where tag_key = 'second_payment_amount');

insert into merge_tags (tag_key, label, group_name, description, is_builtin, source_type, is_active, sort_order)
select 'second_payment_date',
       'Second Payment Date',
       'Event',
       'The due date of the event''s second scheduled payment (blank if none).',
       true, 'builtin', true, 0
where not exists (select 1 from merge_tags where tag_key = 'second_payment_date');
