-- Add the <employee_table4> merge tag — a table of the event's assigned
-- employees with Role, Stage Name, and Phone Number. Keeps the exact DJEP tag
-- name so imported DJEP templates that reference it render correctly.
--
-- Built into render_merge_tags (not a catalog "static/field" tag) because it
-- generates an HTML table from event_staff joined to employees.
...
begin
  ...
  -- <employee_table4>: assigned employees with Role, Stage Name, Phone (DJEP parity)
...
-- Register in the catalog so it appears in the editor's tag list.
insert into merge_tags (tag_key, label, group_name, description, is_builtin, source_type, is_active, sort_order)
select 'employee_table4',
       'Assigned Employees Table (Role, Stage Name, Phone)',
       'Staff',
       'A table of the event''s assigned employees with their role, stage name, and phone number.',
       true, 'builtin', true, 0
where not exists (select 1 from merge_tags where tag_key = 'employee_table4');
