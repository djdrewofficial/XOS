-- Add <djemployee1_stage_name> and <djemployee1_cell_phone> — the event's PRIMARY
-- DJ (first assigned staff, preferring a DJ role). Stage name falls back to the
-- employee's real name; both blank when no staff assigned. Escaped. Built on the
-- escaped render_merge_tags (migration 00149).
...
select * from (values
  ('djemployee1_stage_name', 'DJ 1 — Stage Name', 'Staff',
   'The event''s primary DJ stage name (falls back to their real name); blank if no staff assigned.', true, 'builtin', true, 0),
  ('djemployee1_cell_phone', 'DJ 1 — Cell Phone', 'Staff',
   'The event''s primary DJ cell phone; blank if no staff assigned.', true, 'builtin', true, 0)
