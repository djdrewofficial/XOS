-- Add <third_payment_amount> and <third_payment_date_medium> — the event's THIRD
-- scheduled payment (by seq). Amount as currency; date in medium format
-- (e.g. "Oct 1, 2026"). Blank when there is no third installment.
-- Rebuilds render_merge_tags on top of the escaped version (00145), adding sp3.
...
select 'third_payment_amount', 'Third Payment Amount', 'Event',
       'The amount of the event''s third scheduled payment (blank if none).',
       true, 'builtin', true, 0
where not exists (select 1 from merge_tags where tag_key = 'third_payment_amount');

insert into merge_tags (tag_key, label, group_name, description, is_builtin, source_type, is_active, sort_order)
select 'third_payment_date_medium', 'Third Payment Date (medium)', 'Event',
       'The due date of the event''s third scheduled payment, medium format (e.g. Oct 1, 2026); blank if none.',
       true, 'builtin', true, 0
where not exists (select 1 from merge_tags where tag_key = 'third_payment_date_medium');
