-- XOS — booking helper engine v2 (DJEP parity expansion)
-- New actions: set_event_type, set_event_name (merge tags), set_inquiry_source,
-- set_salesperson, set_custom_date, mark_staff_notified, assign_employee.
-- New: required-field validation before a helper can run.

alter table booking_helpers add column if not exists required_fields text[] not null default '{}';
...
begin
  ...
  -- visibility conditions (server-side enforcement)
  ...
  -- validation: required fields must have values before the helper can run
