-- XOS — enforce a booking helper's event_type_ids scope inside the engine, so a
-- helper restricted to certain event types is skipped on every run path: manual
-- click, the run_helper chain (helper A triggers helper B), and lifecycle
-- triggers. Skip (return) rather than raise, so a chain isn't broken and a
-- manual click on the wrong type just no-ops. Faithful copy of the 00041 engine
-- with one added gate after the visibility checks.
