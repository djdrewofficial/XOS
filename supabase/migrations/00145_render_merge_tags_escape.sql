-- SECURITY: HTML-escape client-controlled values in render_merge_tags.
-- Client-entered fields (names, email, phone, organization, event name, venue,
-- decision-maker/authorized-rep) were substituted into document/email HTML with
-- raw replace() and rendered via dangerouslySetInnerHTML — a stored-XSS path that
-- executes in the master-admin's browser when a generated contract is opened.
-- We now escape every client/venue/event/employee-derived text value. The
-- intentional-HTML pieces stay HTML: the company email signature is left raw, and
-- the builtin table/list tags (employee_table4, addon_list_*, discount,
-- package_description) build their markup from ESCAPED components. Numeric/date/
-- currency/enum values are safe and left as-is.

create or replace function public.xos_html_escape(t text)
 returns text
 language sql
 immutable
...
as $function$
declare
  e record; c record; v record; p record; cs record; sp record; sp2 record; poc record; m record;
...
begin
  ...
  -- Client/venue/event/rep values → escaped.
  ...
  -- Signature is intentional HTML authored by the company — left raw by design.
...
-- SMS path already decodes entities (&amp;→&, etc.) so escaped values render
-- correctly in texts. Pin its search_path while we're here (linter hardening).
alter function public.xos_html_to_sms(text) set search_path = public;
