-- Add the <addon_list_no_prices> merge tag — the event's add-ons by name only
-- (no prices). Uses the client-facing name when set; prefixes a quantity when
-- more than one; one per line so it reads as a list in emails/contracts.
...
declare
  ...
  v_emp_table text; v_addon_list text;
begin
...
select 'addon_list_no_prices',
       'Add-Ons List (names only)',
       'Event',
       'The event''s add-ons by name only, one per line, with no pricing.',
       true, 'builtin', true, 0
where not exists (select 1 from merge_tags where tag_key = 'addon_list_no_prices');
