-- Booking-helper SMS actions become template-based (parity with email actions).
--
-- Previously a helper's SMS action stored a free-typed body. Now it stores a
-- template_id pointing at an SMS template (email_templates where is_sms), and
-- the runtime resolves the template's body_html, fills merge tags, and reduces
-- it to plain text for the SMS. Also adds a send_sms_staff action so a helper
-- can text employees, mirroring send_email_staff.

-- HTML → SMS plain text. Mirrors stripHtml() in src/lib/highlevel.ts so a
-- template authored in the rich editor sends as clean text, not markup. Applied
-- AFTER render_merge_tags: merge tags look like <first_name>, so tags must be
-- substituted before we strip anything that looks like an HTML tag.
create or replace function xos_html_to_sms(html text)
returns text
language plpgsql
immutable
as $$
declare s text := coalesce(html, '');
begin
  s := regexp_replace(s, '<(style|script)[^>]*>.*?</\1>', ' ', 'gis');
  s := regexp_replace(s, '<br\s*/?>', E'\n', 'gi');
  s := regexp_replace(s, '</(p|div|tr|li|h[1-6])>', E'\n', 'gi');
...
begin
  ...
  -- event-type scope gate (added) — skip if restricted and this event doesn't match
    ...
    when 'send_sms' then
      ...
      if v_to is null or v_to = '' then
        ...
        if coalesce(act->>'template_id', '') <> '' then
          -- template-based (new): resolve the SMS template and reduce to plain text
          ...
          -- legacy free-text body (pre-templates helpers) still works
          insert into sms_log (event_id, client_id, to_number, body, status)
          values (p_event_id, e.client_id, v_to,
