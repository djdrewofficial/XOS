-- Add the <discount> merge tag — the event's discount(s) as "Discount: $X",
-- one per line, using the discount's label in parentheses when set. Renders
-- empty when there is no discount so the line disappears in the email/contract.
...
begin
  ...
  -- <discount>: "Discount: $X" per non-zero discount (label in parens when set),
  -- one per line. Empty when there is no discount.
...
select 'discount',
       'Discount',
       'Event',
       'The event''s discount(s) as "Discount: $X" (with the discount label when set); blank when there is no discount.',
       true, 'builtin', true, 0
where not exists (select 1 from merge_tags where tag_key = 'discount');
