-- CRITICAL money fix: <balance_due> / <payments_received> merge tags were
-- summing ALL payment rows, including status='pending' (unarrived Zelle claims)
-- and soft-deleted (deleted_at) rows. Every other balance reader in the app
-- filters status='approved' AND deleted_at is null; this brings render_merge_tags
-- in line so contracts and reminder emails never under-bill a client.
-- Rebuilds render_merge_tags on top of 00151 with ONLY the v_paid line changed.
...
begin
  ...
  -- approved + not-deleted only, matching every other balance reader in the app
