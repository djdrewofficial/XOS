-- Show the <discount> value as a negative ("-$X") so it's visually clear the
-- amount is subtracted from the total (the total math was already subtracting).
-- Only the discount line changes vs 00138.
...
begin
  ...
  -- one per line. Negative sign makes clear it's subtracted. Empty when none.
