-- XOS — SMS via HighLevel (LeadConnector API v2).
-- Outbox pattern mirrors email: helpers queue rows into sms_log,
-- processSmsOutbox() (src/lib/highlevel.ts) upserts the GHL contact and sends;
-- replies land in the GHL Conversations inbox.
...
create table if not exists sms_log (
  ...
  error text,
  hl_contact_id text,          -- HighLevel contact the message was sent to
  provider_message_id text,    -- HighLevel message id
  sent_at timestamptz,
  created_at timestamptz not null default now()
);
create index if not exists sms_log_status_idx on sms_log (status);
create index if not exists sms_log_event_idx on sms_log (event_id);
...
-- ============ BOOKING HELPER EXECUTOR v4 (re-created: + send_sms action) ============
-- Identical to 00023 except the new send_sms case: queues a merge-tagged text
-- to the client's cell (or a custom number) into sms_log.
