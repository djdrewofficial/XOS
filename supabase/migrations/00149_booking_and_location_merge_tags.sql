-- Add merge tags:
--   <booking_comments>          -> events.internal_notes ("Internal Notes / Booking Comments"; staff-only content)
--   <event_location_manager>    -> venues.contact_name
--   <event_location_cell_phone> -> venues.phone
--   <event_location_comments>   -> venues.notes
-- Multi-line notes get newlines -> <br>. All client/venue text is HTML-escaped
-- (built on the escaped render_merge_tags, migration 00148 + sp3).
...
select * from (values
  ('booking_comments', 'Booking Comments', 'Event',
   'The event''s Internal Notes / Booking Comments (staff-only content — avoid in client-facing templates).', true, 'builtin', true, 0),
  ('event_location_manager', 'Event Location — Manager', 'Event',
   'The venue''s on-site contact / manager name.', true, 'builtin', true, 0),
  ('event_location_cell_phone', 'Event Location — Cell Phone', 'Event',
   'The venue''s contact phone number.', true, 'builtin', true, 0),
  ('event_location_comments', 'Event Location — Comments', 'Event',
   'The venue''s notes / comments.', true, 'builtin', true, 0)
