-- Add the <package_price> merge tag — the event's base package price (the price
-- for the selected package only, before add-ons/fees/discounts). Uses the same
-- price precedence as the total-fee math: per-event override, then the locked
-- price, then the package's default. Formatted as currency like <total_fee>.
...
declare
  ...
  v_emp_table text; v_addon_list text; v_pkg_price numeric := 0;
begin
...
select 'package_price',
       'Package Price',
       'Event',
       'The event''s base package price (before add-ons, fees, and discounts).',
       true, 'builtin', true, 0
where not exists (select 1 from merge_tags where tag_key = 'package_price');
