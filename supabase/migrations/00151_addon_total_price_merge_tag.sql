-- Add <addon_total_price>: the total price of all the event's add-ons
-- (quantity x resolved unit price) -- the same v_addons figure already summed
-- into <total_fee>. Rebuilds render_merge_tags on top of 00150 (DJ1 tags) with
-- one added replace, and catalogs the tag so it appears in the template editor
-- tag picker (Money group).
...
select * from (values
  ('addon_total_price', 'Add-Ons Total', 'Money',
   'Total price of all add-ons on the event (quantity x price).', true, 'builtin', true, 0)
