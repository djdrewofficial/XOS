...
export default async function PackagesPage() {
  ...
  const [
    ...
    supabase.from("events").select("package_id"),
    supabase.from("event_addons").select("addon_id"),
  ]);

  // items still attached to events can't be deleted (NO ACTION FK) — hide their delete
  const pkgInUse = new Set((evPkg ?? []).map((e) => e.package_id).filter(Boolean));
  const addonInUse = new Set((evAddon ?? []).map((e) => e.addon_id).filter(Boolean));
  ...
  const uncatAddons = (addons ?? []).filter((a) => !a.category_id || !addonCatIds.has(a.category_id));

  function packageTable(pkgs: NonNullable<typeof packages>) {
    return (
  ...
  function addonTable(items: NonNullable<typeof addons>) {
    return (
      <div className="card overflow-x-auto rounded-t-none">
              ...
              <tr key={a.id} data-searchable className={`row ${!a.is_active ? "opacity-50" : ""}`}>
                ...
                <td className="px-4 py-2 text-right">
                  <div className="flex items-center justify-end gap-2">
                    ...
                    {!addonInUse.has(a.id) && (
                      <DeleteButton
                        action={deleteAddon.bind(null, a.id)}
    ...
    <div className="max-w-6xl" id="packages-root">
      ...
      <div className="mt-2">
        {(addonCategories ?? []).map((cat) => {
          const items = (addons ?? []).filter((a) => a.category_id === cat.id);
        ...
        {uncatAddons.length > 0 && (
          <div className="mb-6" data-search-group>
            <h2 className="mb-0 rounded-t-xl bg-black/[0.07] px-4 py-2 text-sm font-bold uppercase tracking-wide text-zinc-900 dark:bg-white/10 dark:text-white">
              Uncategorized
            </h2>
            {addonTable(uncatAddons)}
          </div>
        )}
      </div>
