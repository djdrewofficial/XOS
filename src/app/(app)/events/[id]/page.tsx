        ...
        <div className="card overflow-hidden">
          {(eventFiles ?? []).map((f) => (
            <div key={f.id} className="flex items-center gap-3 border-t border-zinc-100 px-4 py-2.5 first:border-t-0 dark:border-white/[0.05]">
              ...
              <div className="min-w-0 flex-1">
                <a
                  href={fileLinks.get(f.id) ?? "#"}
                  target="_blank"
                  className="truncate text-sm font-semibold text-brand hover:underline dark:text-brand-lighter"
                >
                  {f.name}
                </a>
                <div className="text-[11px] text-zinc-400">
                  {new Date(f.created_at).toLocaleString()}
                  {f.size_bytes ? ` · ${(Number(f.size_bytes) / 1024).toFixed(0)} KB` : ""}
          ...
          {(eventFiles ?? []).length === 0 && (
            <p className="px-4 py-6 text-center text-sm text-zinc-500">
              No files yet — PDFs attached to outgoing emails are saved here automatically.
            </p>
          )}
        </div>
      </div>
