...
export default function Tabs({
  ...
  return (
    <div>
      <div className="mb-5 flex flex-wrap gap-1 rounded-xl border border-white/[0.07] bg-black/30 p-1 backdrop-blur-sm">
        {tabs.map((t) => (
          <button
            key={t.id}
            ...
            {t.badge !== undefined && t.badge !== 0 && (
              <span
                className={`ml-1.5 rounded-full px-1.5 py-0.5 text-[10px] font-bold ${
                  active === t.id ? "bg-white/25" : "bg-white/10 text-zinc-300"
                }`}
              >
                {t.badge}
              </span>
