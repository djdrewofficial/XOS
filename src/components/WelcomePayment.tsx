...
export default function WelcomePayment(props: WelcomePaymentProps) {
  ...
  const [method, setMethod] = useState<"paypal" | "zelle" | null>(
    both ? null : paypalOk ? "paypal" : zelleOk ? "zelle" : null
  );
    ...
    <div className="space-y-4">
      {both && method === null && (
        <div className="grid gap-3 sm:grid-cols-2">
          <button
            ...
            className="rounded-xl border border-zinc-300 px-4 py-4 text-left transition-colors hover:border-brand hover:bg-brand/5 dark:border-white/15"
          >
      ...
      {method === "paypal" && paypalOk && (
        <div className="space-y-3">
          ...
          />
          {both && (
            <button type="button" onClick={() => setMethod(null)} className="w-full text-center text-xs text-zinc-400 hover:text-zinc-600">
      ...
      {method === "zelle" && zelleOk && (
        <ZellePanel {...props} onBack={both ? () => setMethod(null) : undefined} />
      )}
    </div>
  );
}
    ...
    <div className="space-y-3">
      <div className="rounded-xl border border-zinc-200 bg-zinc-50 px-4 py-4 text-sm dark:border-white/10 dark:bg-white/[0.03]">
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="font-semibold">{zelleDisplayName}</span>
        </div>
        <div className="flex items-center justify-between">
          <span className="text-zinc-500">Zelle</span>
          <span className="font-mono font-semibold">{zelleHandle}</span>
        </div>
        <div className="mt-1 flex items-center justify-between">
          <span className="text-zinc-500">Suggested amount</span>
          <span className="font-bold">{fmt(suggested)}</span>
        </div>
        {balance > suggested + 0.005 && (
          <div className="mt-0.5 text-right text-xs text-zinc-400">or your full balance {fmt(balance)}</div>
        )}
        <div className="mt-2 rounded-md bg-amber-50 px-2.5 py-1.5 text-xs text-amber-800 dark:bg-amber-500/10 dark:text-amber-300">
          {zelleMemo}
        </div>
      </div>
      ...
      <button
        ...
        onClick={async () => {
          setWorking(true);
          try {
            await fetch("/api/pay/zelle-pending", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ token, amount: suggested }),
            });
            setClaimed(true);
          } finally {
            setWorking(false);
      ...
      {onBack && (
        <button type="button" onClick={onBack} className="w-full text-center text-xs text-zinc-400 hover:text-zinc-600">
          ← choose another method
        </button>
      )}
    </div>
